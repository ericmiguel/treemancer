...
class DeclarativeLexerResult:
    ...
    def get_names(self) -> list[DeclarativeToken]:
        """Get all name tokens."""
        return [
            token for token in self.tokens if token.type == DeclarativeTokenType.NAME
        ]

    def get_separators(self) -> list[DeclarativeToken]:
    ...
    def get_cascade_resets(self) -> list[DeclarativeToken]:
    ...
    def get_type_hints(self) -> list[DeclarativeToken]:
        """Get all type hint tokens (d() and f())."""
        return [
            token
            for token in self.tokens
            if token.type
            in {DeclarativeTokenType.DIRECTORY_HINT, DeclarativeTokenType.FILE_HINT}
        ]


@dataclass
class DeclarativeNode:
    """A node in the declarative syntax tree."""

    name: str
    is_directory: bool | None = None  # None = auto-detect, True = dir, False = file
    children: list[DeclarativeNode] | None = None
    ...
    def add_child(self, child: DeclarativeNode) -> None:
        """Add a child node."""
        if self.children is None:
            self.children = []
