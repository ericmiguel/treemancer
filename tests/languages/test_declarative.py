...
class TestDeclarativeLexer:
    ...
    def test_separators_and_groupers(self):
        ...
        expected_types = [
            DeclarativeTokenType.NAME,  # root
            DeclarativeTokenType.SEPARATOR,  # >
            DeclarativeTokenType.NAME,  # src
            DeclarativeTokenType.CASCADE_RESET,  # |
            DeclarativeTokenType.NAME,  # tests
            DeclarativeTokenType.EOF,
        ]
    ...
    def test_type_hints(self):
        ...
        assert tokens[0].type == DeclarativeTokenType.DIRECTORY_HINT
        assert tokens[0].value == "src"
        assert tokens[1].type == DeclarativeTokenType.SEPARATOR
        assert tokens[2].type == DeclarativeTokenType.FILE_HINT
...
class TestDeclarativeIntegration:
    ...
    def test_cascade_reset_beyond_root_error(self):
        ...
        with pytest.raises(
            DeclarativeParseError, match="Cannot cascade reset beyond root"
        ):
